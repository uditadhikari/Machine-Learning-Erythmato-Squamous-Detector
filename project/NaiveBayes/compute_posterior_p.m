...
for i = 1:size(X,1),
	posterior_p_pityriasisRosea(i, :) = 1 ./ ((2 * pi) .* variance_ptyriasisRosea).^ 0.5 .* exp(- ((X(i, :) - expectation_ptyriasisRosea) .^ 2) ./ (2 .* variance_ptyriasisRosea));
end;

for i = 1:size(X,1),
	posterior_p_cronicdermatitis(i, :) = 1 ./ ((2 * pi) .* variance_cronisdermatitis).^ 0.5 .* exp(- ((X(i, :) - expectation_cronisdermatitis) .^ 2) ./ (2 .* variance_cronisdermatitis));
end;

for i = 1:size(X,1),
	posterior_p_pityriasisRubraPilaris(i, :) = 1 ./ ((2 * pi) .* variance_pityriasisRubraPilaris).^ 0.5 .* exp(- ((X(i, :) - expectation_pityriasisRubraPilaris) .^ 2) ./ (2 .* variance_pityriasisRubraPilaris));
end;
